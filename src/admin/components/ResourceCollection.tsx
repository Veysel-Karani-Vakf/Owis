// The list of one record collection (news, projects, banks…), embeddable
// inside a site-page hub tab. Rows open the full record editor at
// /admin/r/:key/:id; that editor's "back" returns to the hub through the
...
import { type FullResourceDef } from '../lib/resources';
...
export default function ResourceCollection({ resource }: { resource: FullResourceDef }) {
  ...
  const toggleVisibility = async (row: Row) => {
    ...
    const next = !row[visibility.column];
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        {resource.description && (
          <p className="w-full text-sm text-slate-500">{resource.description[locale]}</p>
        )}
                        ...
                        <div className="flex flex-wrap items-center gap-1">
                          ...
                          {resource.visibility && (
                            <button
                              ...
                              title={
                                (row[resource.visibility.column]
                                  ? resource.visibility.offLabel
                                  : resource.visibility.onLabel)[locale]
                              }
                              className={
                                'inline-block rounded-full px-2.5 py-0.5 text-xs font-medium transition hover:ring-2 hover:ring-offset-1 ' +
                                (row[resource.visibility.column]
                                  ? 'bg-sky-100 text-sky-700 hover:ring-sky-200'
                                  : 'bg-slate-200 text-slate-600 hover:ring-slate-300')
                              }
                            >
                              {(row[resource.visibility.column]
                                ? resource.visibility.onLabel
                                : resource.visibility.offLabel)[locale]}
                            </button>
                          )}
                        </div>
                      </td>
